...
export async function doctor(profileName, env = process.env) {
  ...
  try {
    ...
    if (profile.upstream.base_url.includes('127.0.0.1') || profile.upstream.base_url.includes('localhost')) add('upstream_not_local_proxy', false, 'upstream base_url should be provider URL, not cpk local proxy');
...
export async function routeTest(profileName, prompt = 'Reply exactly CPK_ROUTE_OK', env = process.env) {
