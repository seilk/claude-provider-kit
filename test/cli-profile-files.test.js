...
test('CLI launches a profile directly without shell aliases or -- separator', async () => {
  ...
  assert.equal(run(['init'], env).status, 0);
  const create = run(['profile', 'create', 'letsur-gpt55', '--provider', 'letsur', '--model', 'gpt-5.5', '--format', 'yaml'], env);
  assert.equal(create.status, 0, create.stderr);
  const launched = run(['letsur-gpt55', '--bare', '-p', 'hi'], env);
  assert.equal(launched.status, 0, launched.stderr);
  const observed = JSON.parse(launched.stdout.trim());
  assert.deepEqual(observed.argv.slice(0, 10), ['--setting-sources', 'project,local', '--settings', observed.argv[3], '--model', 'opus', '--name', 'CPK letsur-gpt55 → gpt-5.5 as claude-opus-4-7', '--bare', '-p']);
  ...
  assert.equal(observed.display, 'CPK letsur-gpt55 → gpt-5.5 as claude-opus-4-7');
});
