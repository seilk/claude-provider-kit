...
test('CLI launches a profile directly without shell aliases or -- separator', async () => {
  ...
  assert.deepEqual(observed.argv.slice(0, 10), ['--setting-sources', 'project,local', '--settings', observed.argv[3], '--model', 'opus', '--name', 'CPK gateway-gpt-4.1 → gpt-4.1 as claude-opus-4-7', '--bare', '-p']);
